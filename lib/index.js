...
class Generator extends EventEmitter {
  constructor(args, options) {
    ...
    require('oe-environment').enforceUpdate(this.env);
  ...
  questions.unshift({
    ...
    validate: function (appName) {
      if (appName.length === 0) {
        return g.f('Application name is mandatory');
      }
      if (appName.charAt(0) === '.') {
        return g.f('Application name cannot start with .: %s', appName);
      }
      if (appName.match(/[\/@\s\+%:]/)) {
        return g.f('Application name cannot contain special characters ' +
          '(/@+%: ): %s', appName);
      }
      if (appName.toLowerCase() === 'node_modules') {
        return g.f('Application name cannot be {{node_modules}}');
      }
      if (appName.toLowerCase() === 'favicon.ico') {
        return g.f('Application name cannot be {{favicon.ico}}');
      }
      if (appName !== encodeURIComponent(appName)) {
  ...
  return this.env.adapter.prompt(questions).then(answers => {
    ...
    this.env.cwd = answers.appName ? path.join(this.env.cwd, answers.appName) : this.env.cwd;
